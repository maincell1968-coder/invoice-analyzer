...
if not st.session_state.autorizzato:
    ...
    **Avviso di Servizio (Fase Beta) e Monitoraggio Accessi**  
    Questo strumento è attualmente ospitato su un server condiviso gratuito per una fase di test riservata a poche aziende selezionate.  
...
<div style='font-size: 0.85em; color: var(--text-color); text-align: justify; padding: 12px; border: 1px solid var(--border-color); border-radius: 5px; background-color: var(--secondary-background-color); opacity: 0.8;'>
    <strong>Disclaimer – Authorized Use and Liability Limitation</strong><br><br>
    This invoice‑analysis program has been designed, developed, and distributed solely by the author for informational and educational purposes.<br><br>
    The software has been independently conceived, designed, and programmed by the author in the Python programming language. No artificial intelligence systems or AI‑based code generation tools were used in the development of this program.<br><br>
    All outputs, analyses, and results generated by the program are non‑official and non‑binding. Users remain solely responsible for verifying the accuracy, completeness, and suitability of any information produced by the program before relying on it for operational or business purposes.<br><br>
    <strong>Authorized Use Only</strong><br>
    Use of this program is strictly limited to companies and entities that have been explicitly authorized by the author. Any use, access, reproduction, distribution, modification, or disclosure of the program—whether in whole or in part—by unauthorized companies or individuals is expressly prohibited.<br><br>
    ...
    The author remains the sole party responsible for the program’s content, functionality, and any consequences arising from its use. No third party is responsible or liable for the software or for any direct or indirect damages resulting from its use.
</div>
""", unsafe_allow_html=True)
